...
# SCPN IEC Core — diagnostic and clock semantics tests
...
from scpn_iec_core.errors import DiagnosticPlanError
...
NONCYCLIC_BINDINGS = {
    ...
    "coordinate_frame": "frm_well_axis",
    "provenance": "synthetic fixture",
    "quality": "synthetic quality flags",
...
REFERENCE_FRAMES = (
    ReferenceFrame(
        ...
        description="polywell field-line following frame",
    ),
    ReferenceFrame(
        identifier="frm_well_axis",
        kind=FrameKind.MACHINE_CYLINDRICAL,
        description="radial well cylindrical frame",
    ),
)
CLOCK_RELATIONS = (
    ClockRelation(
...
SIGNALS_CH_BUNCHING_PROBE = (
    SignalDeclaration(
        identifier="sig_bunch_density_amplitude",
        quantity="number_density",
        unit="m^-3",
        role=SignalRole.AMPLITUDE,
        description="synthetic bunching density amplitude",
    ),
    SignalDeclaration(
...
SIGNALS_CH_LOSS_PROFILE_SET = (
    SignalDeclaration(
        identifier="sig_loss_flux",
        quantity="particle_flux",
        unit="m^-2/s",
        role=SignalRole.CARRIER,
        description="synthetic loss-flux profile",
    ),
    SignalDeclaration(
        identifier="sig_probe_position",
        quantity="length",
        unit="m",
        role=SignalRole.AUXILIARY,
        description="declared probe position",
    ),
)
SIGNALS_CH_STEADY_STATE_SET = (
    SignalDeclaration(
        identifier="sig_grid_current",
        quantity="current",
        unit="A",
        role=SignalRole.AUXILIARY,
        description="synthetic grid current",
    ),
    SignalDeclaration(
        identifier="sig_well_potential",
        quantity="electric_potential",
        unit="V",
        role=SignalRole.CARRIER,
        description="synthetic well potential",
    ),
)
...
REFERENCE_TRANSFORMATIONS: tuple[FrameTransformation, ...] = (
    FrameTransformation(
        ...
        target_identifier="frm_well_axis",
...
def channel_bunching() -> DiagnosticChannelPlan:
    """Build the synthetic electrostatic-bunching probe channel."""
    return DiagnosticChannelPlan(
        identifier="ch_bunching_probe",
        candidate_id="iec.resolved_bunching",
        carrier=SemanticCarrier.COMPLEX_MODE,
        clock_identifier="clk_facility",
        sample_rate_hz=1.0e7,
        max_signal_frequency_hz=1.0e6,
        timing_uncertainty_s=None,
        acquisition_start_s=0.0,
        acquisition_duration_s=100.0,
        element_count=4,
        evidence_bindings=dict(DERIVED_BINDINGS),
        signals=SIGNALS_CH_BUNCHING_PROBE,
        synthetic=True,
    )


def channel_steady_state() -> DiagnosticChannelPlan:
    """Build the synthetic steady-state potential/current channel."""
    return DiagnosticChannelPlan(
        identifier="ch_steady_state_set",
        candidate_id="iec.steady_state",
        ...
        signals=SIGNALS_CH_STEADY_STATE_SET,
        synthetic=True,
    )
...
def channel_loss_profile() -> DiagnosticChannelPlan:
    """Build the synthetic cusp-loss profile channel."""
    return DiagnosticChannelPlan(
        identifier="ch_loss_profile_set",
        ...
        element_count=8,
        evidence_bindings=dict(NONCYCLIC_BINDINGS),
        signals=SIGNALS_CH_LOSS_PROFILE_SET,
        synthetic=True,
    )


def channel_interchange() -> DiagnosticChannelPlan:
...
def synthetic_plan() -> DiagnosticPlan:
    ...
    return DiagnosticPlan(
        ...
        channels=(
            ...
            channel_interchange(),
            channel_loss_profile(),
            channel_steady_state(),
            channel_oscillator(),
        ),
        deferrals=(),
    )
...
def test_candidate_properties_follow_class_tables() -> None:
    ...
    assert drive.configurations == ("polywell",)
    assert SemanticCarrier.CYCLIC_PHASE in drive.admissible_carriers
    assert "diagnostic_reference" in drive.required_evidence
    bunching = by_id["iec.resolved_bunching"]
    assert bunching.configurations == ("gridded_iec", "polywell")
    assert SemanticCarrier.COMPLEX_MODE in bunching.admissible_carriers
...
def test_candidate_rejects_unsorted_configurations() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        CandidateProfile(
            candidate_id="a.b",
            phenomenon="x",
            configurations=("polywell", "gridded_iec"),
            observability_class=ObservabilityClass.NONCYCLIC_FEATURE,
        )
...
def test_candidate_rejects_foreign_configuration() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="not owned"):
        CandidateProfile(
            candidate_id="a.b",
            phenomenon="x",
            configurations=("simple_magnetic_mirror",),
            observability_class=ObservabilityClass.NONCYCLIC_FEATURE,
        )
...
def test_transformation_rejects_self_mapping() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="to itself"):
        _transformation(target_identifier="frm_field_line")
    ...
    with pytest.raises(DiagnosticPlanError, match="must be sorted"):
        _plan_with(
            ...
            frame_transformations=(
                _transformation(
                    source_identifier="frm_well_axis",
...
def test_channel_exposes_observability_class() -> None:
    ...
    assert channel_interchange().observability_class is (
...
def test_report_flags_band_outside_typical_range() -> None:
    ...
    findings = plan.consistency_report()
    assert len(findings) == 1
    assert "Hirsch" in findings[0].message
...
def test_report_flags_clock_coarser_than_sampling() -> None:
    ...
    assert all("cannot distinguish" in finding.message for finding in findings)
...
def test_plan_rejects_unknown_frame_reference() -> None:
    ...
    plan = synthetic_plan()
    kept = tuple(frame for frame in plan.frames if frame.identifier != "frm_well_axis")
...
def test_report_flags_window_beyond_device_ceiling() -> None:
    ...
    channel = _derived(acquisition_duration_s=10000.0)
    plan = synthetic_plan()
    plan = DiagnosticPlan(
        identifier=plan.identifier,
        binding=plan.binding,
