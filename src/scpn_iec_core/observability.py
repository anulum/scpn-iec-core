...
# SCPN IEC Core — diagnostic and clock semantics
...
identities for the electrostatic-confinement configurations this
repository owns,
...
from scpn_iec_core.configuration import ConsistencyFinding
...
BUNCHING_BAND_HZ: Final = (1.0e4, 1.0e7)
        ...
        for configuration in self.configurations:
            if configuration not in OWNED_CONFIGURATIONS:
                raise DiagnosticPlanError(
                    ...
                    f"{configuration!r} is not owned by SCPN-IEC-CORE"
                )
...
APPLICABLE_CANDIDATES: Final = (
    CandidateProfile(
        ...
        phenomenon=("resolved electrostatic density or potential oscillation"),
        configurations=("gridded_iec", "polywell"),
        observability_class=ObservabilityClass.DERIVED_CYCLIC,
    ),
    CandidateProfile(
        candidate_id="iec.steady_state",
        phenomenon="potential, density, current, and fusion-yield state",
        configurations=("gridded_iec", "polywell"),
...
class DiagnosticChannelPlan:
    ...
    timing_uncertainty_s
        ...
        candidate is applicable to SCPN-IEC-CORE.
    acquisition_start_s
            ...
            if channel.candidate_id == "iec.resolved_bunching" and not (
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            "declared bunching band "
                            f"{channel.max_signal_frequency_hz:.1f} "
                            "Hz is outside the electrostatic bunching "
                            "scale 10 kHz-10 MHz (Hirsch 1967)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            f"{channel.max_signal_frequency_hz:.1f} "
                            "Hz is outside the RF drive range "
                            "1 MHz-200 GHz used on electrostatic devices "
                            "(Hirsch 1967)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "longer than the IEC steady-run scale of up to "
                            "~1000 s (Hirsch 1967)"
                        ),
                    )
                )
