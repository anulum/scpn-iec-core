...
# SCPN IEC Core — device configuration container

"""Device configuration container bound to the SPO reactor registry.

A :class:`DeviceConfiguration` composes a validated confinement
structure and electrostatic drive under exactly one of the two registry
identifiers this repository owns. The grid/virtual-cathode class
invariants are hard (Hirsch, JAP 38 (1967) 4522; Bussard's polywell
line); a well voltage below the documented IEC operating range is
flagged. Serialisation is canonical (sorted keys, no NaN or infinity
...
OWNED_CONFIGURATIONS: Final = ("gridded_iec", "polywell")
KIND_BY_IDENTIFIER: Final = {
    "gridded_iec": "gridded",
    "polywell": "polywell",
}
IEC_MIN_OPERATING_KV: Final = 20.0
...
class DeviceConfiguration:
    """Validated IEC device configuration.

    Parameters
    ----------
    identifier
        SPO registry configuration identifier; one of ``gridded_iec``
        or ``polywell``.
    confinement
        Validated confinement structure.
    drive
        Validated electrostatic drive.
    registry
    ...
    DeviceConfigurationError
        ...
        confinement class contradicts it.
    """
    ...
    def __post_init__(self) -> None:
        """Validate identifier ownership and the class invariant.
        ...
        if self.identifier not in OWNED_CONFIGURATIONS:
            raise DeviceConfigurationError(
                ...
                f"SCPN-IEC-CORE; owned: {OWNED_CONFIGURATIONS!r}"
            )
        expected = KIND_BY_IDENTIFIER[self.identifier]
        if self.confinement.kind != expected:
            raise DeviceConfigurationError(
                f"confinement.kind: {self.identifier} requires the "
                f"{expected!r} confinement class, "
                f"got {self.confinement.kind!r}"
            )
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            ...
            the well voltage sits in the documented IEC operating
            range. Findings are advisory instruments, not machine
            claims.
        """
        findings: list[ConsistencyFinding] = []
        voltage = self.drive.well_voltage_kv
        if voltage < IEC_MIN_OPERATING_KV:
            findings.append(
                ConsistencyFinding(
                    field="drive.well_voltage_kv",
                    message=(
                        f"well voltage {voltage:.1f} kV is below the "
                        f"documented IEC operating floor "
                        f"{IEC_MIN_OPERATING_KV:.0f} kV"
                    ),
                )
            )
    ...
    def to_record(self) -> dict[str, Any]:
        ...
        return {
            ...
            "confinement": {
                "kind": self.confinement.kind,
                "cathode_grid_transparency": (
                    self.confinement.cathode_grid_transparency
                ),
                "polyhedral_coil_count": (self.confinement.polyhedral_coil_count),
            },
            "drive": {
                "well_voltage_kv": self.drive.well_voltage_kv,
            },
            "registry": {
...
def configuration_from_record(record: Any) -> DeviceConfiguration:
    ...
    known = {"identifier", "confinement", "drive", "registry"}
    unknown = sorted(set(record) - known)
    if unknown:
        raise DeviceConfigurationError(f"record: unknown fields {unknown!r}")
    confinement = _require_mapping(record, "confinement")
    drive = _require_mapping(record, "drive")
    registry = _require_mapping(record, "registry")
    return DeviceConfiguration(
        identifier=_string(record, "identifier"),
        confinement=ConfinementStructure(
            kind=_string(confinement, "kind"),
            cathode_grid_transparency=_number(confinement, "cathode_grid_transparency"),
            polyhedral_coil_count=_integer(confinement, "polyhedral_coil_count"),
        ),
        drive=ElectrostaticDrive(
            well_voltage_kv=_number(drive, "well_voltage_kv"),
        ),
        registry=RegistryBinding(
