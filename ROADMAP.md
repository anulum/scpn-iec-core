...
- **Device configuration model** (landed 2026-08-31) — validated
  confinement and drive objects for `gridded_iec` and `polywell` with
  hard grid/virtual-cathode class invariants (grid transparency vs
  polyhedral coil count), the ion-energy relation `E = e U`
  (Hirsch 1967), an operating-voltage advisory, canonical digests, and
  the SPO registry data pin; `computational_prototype` (ADR 0002,
  `VALIDATION.md#device-configuration-model`). Well-depth envelopes and
  discharge modes remain future work under the same capability.
- **Diagnostic and clock semantics** (landed 2026-08-31) — synthetic
  ...
  applicability (open.* candidates polywell-scoped), carrier
  admissibility, exact evidence vocabularies, clock-kind compatibility,
  Nyquist bounds, canonical digests; the reference plan mirrors
  canonical practice (bunching probe, steady-state set, drive reference,
  loss-profile set, interchange probe array, synthetic oscillator);
