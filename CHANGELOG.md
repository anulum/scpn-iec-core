...
- Generated repository header artwork: `docs/assets/generate_header.py`
  ...
  domain surface (the electrostatic potential well used by the README,
  the gridded/polywell class split, and the well-voltage range).
...
- Diagnostic and clock semantics model
  (`src/scpn_iec_core/observability.py`), the second implemented
  ...
  observability-profile catalogue (candidate applicability with
  polywell-scoped open.* candidates, carrier admissibility, exact
  class-fixed evidence vocabularies, clock-kind compatibility, Nyquist
  bounds); cited advisory band checks; canonical serialisation with
  SHA-256 digests and strict NaN-rejecting round-trip parsing (design
  record `docs/adr/0003-diagnostic-clock-semantics.md`).

- Device configuration model (`src/scpn_iec_core/`), the first implemented
